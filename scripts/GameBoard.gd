...
signal end_turn
signal create_kings(tile, piece_name, color)
signal considering_move(piece)
signal considering_pawn_sacrifice(piece)
...
var NEW_PIECE = null
var PENDING_ACTION = null #move or create
...
func _ready(): #create initial board state
	...
	for x in range(8):
		...
		for y in range(3):
			...
			# Place Kings
			if x == 0 or x == 7:
				create_kings.emit(newBoardTile, "King", "White" if (x == 0) else "Black")
		
		boardTiles.push_back(newRow)


func resetMoveTiles():
	var board_tiles = get_tree().get_nodes_in_group("Board Tiles")
	for tile in board_tiles:
		tile.toggle_button_off()
...
func activate_spawn_columns(col_numbers: Array):
	resetMoveTiles()
...
func _handle_chesspiece_clicked(piece):
	...
	if piece == NEW_PIECE: return
	if not Data.player_turn == piece.color: return
	
	resetMoveTiles()
	
	considering_move.emit(piece)
	...
	if piece.type_of_piece == "Pawn" and inBackRow.call():
		considering_pawn_sacrifice.emit(piece)


func _handle_board_tile_button_clicked(tile):
	PENDING_ACTION.call(tile)
	resetMoveTiles()
